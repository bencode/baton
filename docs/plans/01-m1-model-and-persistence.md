...
server 作为常驻进程持有这套状态。这是后续认领调度、worker 协作、自举的承重墙。

## 范围 / 非范围

**M1 范围**：静态协同模型（Workspace/Project/Requirement/Task）+ 纯派生函数、
Prisma+SQLite 持久化（Store port + 契约 harness）、server 骨架（Node + 最小 HTTP CRUD + 优雅启停）。
...
baton/
  ...
  packages/
    ...
    server/   @baton/server   Node 运行时（tsx）；Prisma + Store + HTTP；持有状态
```
...
## server 骨架（`@baton/server`，Node + Hono）
...
- server 自己的 harness：boot→`/health`→建链→读回→关停，验证"进程+Store+HTTP"贯通（含真实 node server 启停）。
...
```
pnpm install
pnpm --filter @baton/server exec prisma migrate dev --name init   # 已生成 init 迁移
pnpm check     # biome + typecheck + test（tsx --test / node:test）一条龙
```

## 运行时与持久化选型（M1 实测结论）

- **运行时用 Node，不用 Bun**：Bun 偏构建期/CLI 工具；长期运行的服务端进程走 Node 更稳、与 tcollab 同栈。
...
- **两端**：server（服务端，持有状态/API）；CLI（客户端），两种消费模式：
