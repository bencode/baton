...
// The current workspace's chat rooms (membership-gated). Mirrors useProjects —
// re-keyed by useListRevision so a create/rename refetches.
export const useChannels = (workspaceId: Id | null) => {
  const api = useApi()
  const rev = useListRevision()
  return useAsync<Channel[]>(
