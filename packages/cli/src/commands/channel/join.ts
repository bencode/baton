...
export const joinCommand = defineCommand({
  meta: {
    name: 'join',
    description: 'join a channel (register presence); --listen to also stream',
  },
  args: {
    ...
    listen: {
      type: 'boolean',
      description: 'after joining, stream messages (background subscriber)',
    },
  ...
  run: async ({ args }) => {
    ...
    const members = await channelClient(url).join(args.channel, args.from, kind)
    if (!args.listen) {
      if (args.json) console.log(toJson({ joined: args.from, members }))
      else
        console.log(`joined as ${args.from} (${members.length} online)\n${renderRoster(members)}`)
      return
    }
