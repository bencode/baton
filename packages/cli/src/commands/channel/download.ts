...
// attachment to a local file. Agents can equally `curl -H "authorization: Bearer
// $TOK" "$BASE/channels/$CH/attachments/$ATT" -o file`; this just saves the typing.
export const downloadCommand = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    const res = await channelClient(url).downloadAttachment(args.channel, args.token, args.att)
