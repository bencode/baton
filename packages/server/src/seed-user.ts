...
// Create one back-office login user (admin — sees every workspace). Creating the
// first user is what flips the server into authed mode — the cookie gate enforces
// once ≥1 user exists. Safe to re-run: an existing username is left untouched.
// Non-admin members are made by binding (see user-bind.ts).
...
if (existing) {
  ...
  console.log(`created admin user "${username}" (id=${user.id}) — back-office auth is now enforced`)
}
await store.close()
